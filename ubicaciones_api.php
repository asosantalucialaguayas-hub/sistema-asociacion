...
// Recibe el KML generado en el mini-QGIS y lo guarda como
// si fuera un archivo subido normalmente.
// ────────────────────────────────────────────────────────────
...
$check = $pdo->prepare("SELECT id_ubicacion FROM socio_ubicaciones WHERE codigo_archivo = ?");
    ...
    // Directorio de almacenamiento (igual que en la subida normal)
    $uploadDir = __DIR__ . '/uploads/kml/';
    if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
    ...
    // Ruta relativa para la BD (igual que el sistema actual)
    ...
    try {
        ...
        // Si falló la BD, borrar el archivo
        @unlink($filepath);
    ...
    break;
        ...
        case 'exportar_socio':
            ...
            $st = $pdo->prepare("
                SELECT u.*, s.identificacion,
                    COALESCE(s.nombre_completo,
                        CONCAT(COALESCE(s.nombres,''),' ',COALESCE(s.apellidos,''))) AS nombre_socio
                FROM socio_ubicaciones u LEFT JOIN socios s ON s.id_socio = u.id_socio
            ...
            foreach ($archivos as $a) {
                ...
                $nombreEnZip = $a['codigo_archivo']
                    ? $a['codigo_archivo'] . '.' . $a['tipo_archivo']
                    : $a['nombre_archivo'];
        ...
        case 'exportar_todos':
            ...
            foreach ($archivos as $a) {
                $ruta = __DIR__ . '/' . $a['ruta_archivo'];
                if (!file_exists($ruta)) continue;
                $carpeta = ($a['identificacion'] ?? 'sin_cedula') . '_'
                         . preg_replace('/[^a-zA-Z0-9_]/','_',trim($a['nombre_socio'] ?? ''));
                $nombreEnZip = $a['codigo_archivo']
                    ? $a['codigo_archivo'] . '.kml'
                    : $a['nombre_archivo'];

                // Inyectar atributos actualizados en el KML antes de zipar
                $contenido = file_get_contents($ruta);
                if ($a['tipo_archivo'] === 'kmz') {
                    $zipKmz = new ZipArchive();
                    if ($zipKmz->open($ruta) === true) {
                        for ($ki = 0; $ki < $zipKmz->numFiles; $ki++) {
                            $nz = $zipKmz->getNameIndex($ki);
                            if (strtolower(pathinfo($nz, PATHINFO_EXTENSION)) === 'kml') {
                                $contenido = $zipKmz->getFromIndex($ki); break;
                            }
                        }
                        $zipKmz->close();
                    }
                ...
                if (!empty($a['atributos'])) {
                    ...
                    $titulo = $a['titulo_aviso'] ?: $a['codigo_archivo'] ?: '';
                    $rows = '';
                    foreach ($atrs as $i => $atr) {
                        $bg = $i % 2 !== 0 ? ' bgcolor="#D4E4F3"' : '';
                        $rows .= "<tr{$bg}><td>".htmlspecialchars($atr['k'])."</td><td>".htmlspecialchars($atr['v'])."</td></tr>";
                    }
                    $htmlDesc = "<table style=\"font-family:Arial;font-size:12px;width:100%;border-collapse:collapse;\"><tr style=\"background:#9CBCE2;font-weight:bold;text-align:center;\"><td colspan=\"2\">".htmlspecialchars($titulo)."</td></tr>{$rows}</table>";
                    $newDesc = "<description><![CDATA[\n{$htmlDesc}\n]]></description>";
                    if (preg_match('/<description/i', $contenido))
                        $contenido = preg_replace('/<description[\s\S]*?<\/description>/i', $newDesc, $contenido);
                    else
                        $contenido = preg_replace('/(<Placemark[^>]*>)/i', '$1'.$newDesc, $contenido);
                }
        ...
        // ═══════════════════════════════════════════════════════════════════
        // EXPORTAR EXCEL — Una fila por KML: Cédula | Nombre | Código | ...
        // ═══════════════════════════════════════════════════════════════════
        case 'exportar_excel':
            $q         = trim($_GET['q'] ?? '');
            $con_kml   = trim($_GET['con_kml'] ?? '');
            ...
            $headerCols = [
                'A1'=>'CÉDULA',           'B1'=>'NOMBRE PRODUCTOR/A',
                'C1'=>'ZONA',             'D1'=>'COMUNIDAD',
                'E1'=>'ADENDUM',          'F1'=>'CÓDIGO KML',
                'G1'=>'ARCHIVO',          'H1'=>'TIPO',
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
          xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
    <sheet name="Ubicaciones KML" sheetId="1" r:id="rId1"/>
  </sheets>
</workbook>');
  ...
  <Relationship Id="rId2"
    ...
    Target="styles.xml"/>
        ...
        case 'exportar_resumen_excel':
            ...
            $sheetXml2 = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetFormatPr defaultRowHeight="15"/>
  <cols>'.$colsXml.'</cols>
  <sheetData>'.$rowsXml.'</sheetData>
  <autoFilter ref="A2:F2"/>
            ...
            $stylesXml2 = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <fonts count="2">
    <font><sz val="11"/><name val="Arial"/></font>
    <font><sz val="11"/><b/><color rgb="FFFFFFFF"/><name val="Arial"/></font>
  ...
  <cellXfs count="2">
    ...
    <xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1"/>
            ...
            $zip2->addFromString('[Content_Types].xml',
'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  <Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>
  <Default Extension="xml" ContentType="application/xml"/>
  ...
  <Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/>
            ...
            $zip2->addFromString('_rels/.rels',
'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/>
</Relationships>');
            $zip2->addFromString('xl/workbook.xml',
'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets><sheet name="Lotes KML" sheetId="1" r:id="rId1"/></sheets>
</workbook>');
            $zip2->addFromString('xl/_rels/workbook.xml.rels',
'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  <Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>
        ...
        // ═══════════════════════════════════════════════════════════════════
        // BLOQUE 4 — EXPORTAR RESUMEN GLOBAL EXCEL
        // ═══════════════════════════════════════════════════════════════════
        case 'exportar_resumen_global_excel':
            ...
            $sheetXmlG = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetFormatPr defaultRowHeight="15"/>
  <cols>'.$colsXml.'</cols>
  <sheetData>'.$rowsXml.'</sheetData>
  <autoFilter ref="A1:G1"/>
            ...
            $stylesXmlG = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <fonts count="2">
    <font><sz val="11"/><name val="Arial"/></font>
    <font><sz val="11"/><b/><color rgb="FFFFFFFF"/><name val="Arial"/></font>
  ...
  <cellXfs count="2">
    ...
    <xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1"/>
            ...
            $zip3->addFromString('[Content_Types].xml',
'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  <Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>
  <Default Extension="xml" ContentType="application/xml"/>
  ...
  <Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/>
            ...
            $zip3->addFromString('_rels/.rels',
'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/>
</Relationships>');
            $zip3->addFromString('xl/workbook.xml',
'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets><sheet name="Resumen Global KML" sheetId="1" r:id="rId1"/></sheets>
</workbook>');
            $zip3->addFromString('xl/_rels/workbook.xml.rels',
'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/>
  <Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>
        ...
        // ═══════════════════════════════════════════════════════════════════
        // ACTUALIZAR KML EDITADO — reemplaza el archivo físico en servidor
        // Llamado desde el editor de polígonos de mapa_global.php
        // ═══════════════════════════════════════════════════════════════════
        case 'actualizar_kml_editado':
            ...
            if (empty($kml_content)) {
                echo json_encode(['success'=>false,'message'=>'Contenido KML vacío']); exit;
            }
            ...
            // Determinar ruta física destino
            $rutaFisica = __DIR__ . '/' . $row['ruta_archivo'];
            $rutaKml    = $rutaFisica;

            // Si el original es KMZ, guardar como .kml paralelo y actualizar BD
            ...
            // Extraer hectáreas del atributo tipo area
            $nuevaHa = null;
            foreach ($atrsDecoded as $atr) {
                if (($atr['tipo'] ?? '') === 'area') {
                    $nuevaHa = floatval($atr['v'] ?? 0);
                    break;
                }
            }
            ...
            // Actualizar atributos y auditoría en BD
            $pdo->prepare("UPDATE socio_ubicaciones SET
                atributos     = :atributos,
                editado_por   = :usuario,
                fecha_edicion = NOW()
            WHERE id_ubicacion = :id")
            ->execute([
                ':atributos' => json_encode($atrsLimpios, JSON_UNESCAPED_UNICODE),
            ...
            echo json_encode([
                'success'     => true,
                'message'     => 'KML actualizado correctamente',
                'hectareas'   => $nuevaHa,
        ...
        case 'descargar_kml_actualizado':
            ...
            $row = $st->fetch(PDO::FETCH_ASSOC);
            if (!$row) { exit; }
            $contenido = file_get_contents(__DIR__.'/'.$row['ruta_archivo']);
            if (!empty($row['atributos'])) {
                $atrs = json_decode($row['atributos'], true) ?: [];
                $titulo = $row['titulo_aviso'] ?: $row['codigo_archivo'] ?: '';
                $rows = '';
                foreach ($atrs as $i => $a) {
                    $bg = $i % 2 !== 0 ? ' bgcolor="#D4E4F3"' : '';
                    $rows .= "<tr{$bg}><td>".htmlspecialchars($a['k'])."</td><td>".htmlspecialchars($a['v'])."</td></tr>";
                }
                $htmlDesc = "<table style=\"font-family:Arial;font-size:12px;width:100%;border-collapse:collapse;\"><tr style=\"background:#9CBCE2;font-weight:bold;text-align:center;\"><td colspan=\"2\">".htmlspecialchars($titulo)."</td></tr>{$rows}</table>";
                $newDesc = "<description><![CDATA[\n{$htmlDesc}\n]]></description>";
                if (preg_match('/<description/i', $contenido))
