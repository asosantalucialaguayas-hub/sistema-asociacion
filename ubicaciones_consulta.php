...
.rg-stat.violeta p{color:#7c3aed}.rg-stat.verde p{color:#10b981}.rg-stat.ambar p{color:#f59e0b}.rg-stat.rojo p{color:#ef4444}
...
.rg-tc{display:none;padding:0}.rg-tc.active{display:block}
.rg-table{width:100%;border-collapse:collapse;font-size:12.5px}
.rg-table thead th{background:#1f3a5f;color:#fff;padding:10px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.3px;white-space:nowrap;position:sticky;top:0;z-index:5}
...
.ic-close:hover{background:rgba(255,255,255,.3)}
.ic-stats{display:flex;gap:10px;padding:12px 20px;background:#f8fafc;border-bottom:1px solid #e5e7eb;flex-shrink:0;flex-wrap:wrap}
.ic-stat{background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:8px 16px;text-align:center;flex:1;min-width:100px}
.ic-stat h6{margin:0 0 2px;font-size:10px;color:#6b7280;text-transform:uppercase;letter-spacing:.5px}
.ic-stat p{margin:0;font-size:18px;font-weight:700;color:#1f3a5f}
.ic-stat.verde p{color:#10b981}.ic-stat.rojo p{color:#ef4444}.ic-stat.ambar p{color:#f59e0b}.ic-stat.azul p{color:#0891b2}
.ic-toolbar{display:flex;gap:10px;align-items:center;padding:12px 20px;border-bottom:1px solid #e5e7eb;flex-wrap:wrap;flex-shrink:0;background:#fff}
.ic-search{padding:8px 12px;border-radius:8px;border:1px solid #d1d5db;font-size:13px;min-width:200px;flex:1;max-width:320px}
.ic-search:focus{outline:none;border-color:#0891b2}
.ic-filtros{display:flex;gap:6px;flex-wrap:wrap}
.ic-filtro-btn{padding:6px 14px;border-radius:20px;border:1.5px solid #e5e7eb;background:#fff;cursor:pointer;font-size:12px;font-weight:600;color:#6b7280;transition:all .15s}
.ic-filtro-btn:hover{border-color:#0891b2;color:#0891b2}
.ic-filtro-btn.activo{background:#0891b2;color:#fff;border-color:#0891b2}
.ic-body{flex:1;overflow-y:auto;min-height:0}
.ic-body::-webkit-scrollbar{width:5px}
.ic-body::-webkit-scrollbar-thumb{background:#d1d5db;border-radius:4px}
.ic-table{width:100%;border-collapse:collapse;font-size:12.5px}
.ic-table thead th{background:#0f766e;color:#fff;padding:10px 12px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.3px;white-space:nowrap;position:sticky;top:0;z-index:5}
.ic-table td{padding:10px 12px;border-bottom:1px solid #f1f5f9;vertical-align:middle}
.ic-table tbody tr:hover{background:#f8fafc}
.ic-badge{display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:999px;font-size:11px;font-weight:700;white-space:nowrap}
.ic-badge.igual{background:#d1fae5;color:#065f46}
.ic-badge.exceso{background:#fef3c7;color:#92400e}
.ic-badge.deficit{background:#fee2e2;color:#991b1b}
.ic-badge.sin-kml{background:#f3f4f6;color:#6b7280}
.ic-ha{font-family:monospace;font-weight:700;color:#1f3a5f}
.ic-ha-nd{color:#9ca3af;font-style:italic;font-size:11px}
.ic-table tbody tr.row-deficit{background:#fff5f5}
.ic-table tbody tr.row-deficit:hover{background:#fee2e2}
.ic-table tbody tr.row-exceso{background:#fffbeb}
.ic-table tbody tr.row-exceso:hover{background:#fef3c7}
.ic-table tbody tr.row-igual{background:#f0fdf4}
.ic-table tbody tr.row-igual:hover{background:#dcfce7}
.ic-foot{padding:12px 20px;border-top:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-shrink:0;background:#f9fafb;flex-wrap:wrap}
.ic-loading{text-align:center;padding:40px;color:#6b7280;font-size:14px}
.ic-leyenda{display:flex;gap:12px;flex-wrap:wrap;font-size:11px;color:#6b7280}
.ic-leyenda span{display:flex;align-items:center;gap:5px}
.ic-dot{width:10px;height:10px;border-radius:50%;display:inline-block;flex-shrink:0}
</style>
</head>
<body>
...
<div id="tabArchivos" class="tab-content active">
    <div style="display:flex;justify-content:flex-end;margin-bottom:10px;">
        <button class="btn-warning" onclick="exportarSocio()"><i class="fa fa-file-zipper"></i> Exportar archivos de este socio (ZIP)</button>
    </div>
    <div id="listaArchivos"><p style="text-align:center;color:#9ca3af;padding:20px;"><i class="fa fa-spinner fa-spin"></i> Cargando...</p></div>
</div>
...
<div id="resumenContenido"><div class="resumen-loading"><i class="fa fa-spinner fa-spin" style="font-size:24px;color:#3b82f6;margin-bottom:8px;display:block;"></i>Cargando resumen...</div></div>
<div style="display:flex;gap:10px;margin-top:16px;justify-content:flex-end;flex-wrap:wrap;">
    <button class="btn-success" onclick="exportarResumenExcel()" id="btnExportarResumen" style="display:none;"><i class="fa fa-file-excel"></i> Exportar a Excel</button>
    <button class="btn-secondary" onclick="cerrarModal('modalResumen')"><i class="fa fa-times"></i> Cerrar</button>
</div>
</div>
</div>
...
<div id="modalResumenGlobal" class="rg-overlay">
  <div class="rg-box">
    ...
    <div class="rg-tabs">
      <button class="rg-tab active" onclick="cambiarRgTab('rgTabSocios',this)"><i class="fa fa-users"></i> Socios y Hectáreas</button>
      <button class="rg-tab" onclick="cambiarRgTab('rgTabLibres',this)"><i class="fa fa-circle-check"></i> Códigos Disponibles</button>
    </div>
    ...
    <div class="rg-body">
      ...
      <div id="rgTabLibres" class="rg-tc"><div id="rgContenidoLibres"><div class="rg-loading"><i class="fa fa-spinner fa-spin" style="font-size:24px;color:#10b981;display:block;margin-bottom:8px;"></i>Calculando códigos...</div></div></div>
    </div>
    <div class="rg-foot">
      <button onclick="cerrarResumenGlobal()" style="padding:8px 18px;border-radius:8px;border:1px solid #d1d5db;background:#fff;cursor:pointer;font-size:13px;font-weight:600;color:#374151;"><i class="fa fa-times"></i> Cerrar</button>
    </div>
  </div>
</div>
...
<div id="modalComparacion" class="ic-overlay">
  <div class="ic-box">
    <div class="ic-head">
      <h2><i class="fa fa-scale-balanced"></i> Área Cacao LPA vs Hectáreas KML</h2>
      <button class="ic-close" onclick="cerrarComparacion()"><i class="fa fa-times"></i></button>
    </div>
    <div class="ic-stats">
      <div class="ic-stat"><h6>Total socios</h6><p id="icStTotal">—</p></div>
      <div class="ic-stat verde"><h6>Igualados ✓</h6><p id="icStIgual">—</p></div>
      <div class="ic-stat rojo"><h6>Déficit KML</h6><p id="icStDeficit">—</p></div>
      <div class="ic-stat ambar"><h6>Exceso KML</h6><p id="icStExceso">—</p></div>
      <div class="ic-stat azul"><h6>Sin KML</h6><p id="icStSinKml">—</p></div>
    </div>
    <div class="ic-toolbar">
      <input type="text" class="ic-search" id="icBuscar" placeholder="🔍 Buscar por cédula o nombre..." oninput="filtrarComparacion()">
      <div class="ic-filtros">
        <button class="ic-filtro-btn activo" onclick="setFiltroIc(this,'todos')">Todos</button>
        <button class="ic-filtro-btn" onclick="setFiltroIc(this,'igual')">✅ Igualados</button>
        <button class="ic-filtro-btn" onclick="setFiltroIc(this,'deficit')">🔴 Déficit</button>
        <button class="ic-filtro-btn" onclick="setFiltroIc(this,'exceso')">🟡 Exceso</button>
        <button class="ic-filtro-btn" onclick="setFiltroIc(this,'sin-kml')">⬜ Sin KML</button>
      </div>
    </div>
    <div class="ic-body"><div id="icContenido"><div class="ic-loading"><i class="fa fa-spinner fa-spin" style="font-size:28px;color:#0891b2;display:block;margin-bottom:10px;"></i>Cargando...</div></div></div>
    <div class="ic-foot">
      <div class="ic-leyenda">
        <span><span class="ic-dot" style="background:#10b981;"></span> Igualados (diff ≤ 0.05 ha)</span>
        <span><span class="ic-dot" style="background:#ef4444;"></span> Déficit KML</span>
...
async function abrirResumen(idSocio,cedula,nombre,zona,codigoSlc,totalArchivos){
    ...
    document.getElementById('resumenContenido').innerHTML='<div class="resumen-loading"><i class="fa fa-spinner fa-spin" style="font-size:24px;color:#3b82f6;margin-bottom:8px;display:block;"></i>Cargando resumen...</div>';
    document.getElementById('modalResumen').classList.add('active');
    let archivos=[];
    try{const r=await fetch(`ubicaciones_api.php?accion=listar&id_socio=${idSocio}`);const j=await r.json();if(j.success)archivos=j.datos;}catch(e){}
...
function calcularAreaDesdeKml(kmlStr){
    try{
        ...
        if(descEl){const content=descEl.textContent||'';const dd=new DOMParser().parseFromString(content,'text/html');const rows=dd.querySelectorAll('tr');for(const row of rows){const tds=row.querySelectorAll('td');if(tds.length>=2){const k=(tds[0].textContent||'').toLowerCase();if(k.includes('area')||k.includes('área')||k.includes('hectarea')||k==='ha'){const v=parseFloat((tds[1].textContent||'').replace(',','.'));if(!isNaN(v))return v;}}}}
        const simpleDatas=doc.querySelectorAll('SimpleData');for(const sd of simpleDatas){const n=(sd.getAttribute('name')||'').toLowerCase();if(n.includes('area')||n.includes('área')||n.includes('hectarea')){const v=parseFloat((sd.textContent||'').replace(',','.'));if(!isNaN(v))return v;}}
        const datas=doc.querySelectorAll('ExtendedData > Data');for(const d of datas){const n=(d.getAttribute('name')||'').toLowerCase();if(n.includes('area')||n.includes('área')||n.includes('hectarea')){const valEl=d.querySelector('value');if(valEl){const v=parseFloat((valEl.textContent||'').replace(',','.'));if(!isNaN(v))return v;}}}
        const coordEls=doc.querySelectorAll('coordinates');if(!coordEls.length)return null;
        let allCoords=[];coordEls.forEach(el=>{(el.textContent||'').trim().split(/\s+/).forEach(c=>{const p=c.split(',');if(p.length>=2){const lon=parseFloat(p[0]),lat=parseFloat(p[1]);if(!isNaN(lon)&&!isNaN(lat))allCoords.push([lat,lon]);}});});
        if(allCoords.length>3){let area=0;const n2=allCoords.length,R=6371000;for(let i=0;i<n2-1;i++){const lat1=allCoords[i][0]*Math.PI/180,lat2=allCoords[i+1][0]*Math.PI/180;const dlon=(allCoords[i+1][1]-allCoords[i][1])*Math.PI/180;area+=dlon*(2+Math.sin(lat1)+Math.sin(lat2));}return parseFloat((Math.abs(area)*R*R/2/10000).toFixed(3));}
    }catch(e){}return null;
}
...
function renderResumen(data){
    ...
    data.lotes.forEach((l,i)=>{
        const haStr=l.hectareas!==null&&!isNaN(l.hectareas)?`<span class="hectareas-val">${parseFloat(l.hectareas).toFixed(3)}</span>`:`<span class="hectareas-nd">Sin datos</span>`;
...
function exportarResumenExcel(){
    ...
    const payload={cedula:d.cedula,nombre:d.nombre,zona:d.zona,codigoSlc:d.codigoSlc,lotes:d.lotes,totalHa:d.totalHa};
    const form=document.createElement('form');form.method='POST';form.action='ubicaciones_api.php';form.target='_blank';
    const i1=document.createElement('input');i1.type='hidden';i1.name='accion';i1.value='exportar_resumen_excel';
...
function abrirUbicaciones(idSocio,cedula,nombre,zona,comunidad,codigoSlc,proximoLote){
    ...
    capasKml={};document.getElementById('listaCapasMapa').innerHTML='';
...
function onArchivosSeleccionados(files){
    ...
    if(socioActual)socioActual.proximoLote=lote;renderCola();document.getElementById('archivoInput').value='';
}

function renderCola(){
    const div=document.getElementById('colaArchivos'),btnActions=document.getElementById('bulkActions');
    ...
    colaFiles.forEach((item,idx)=>{
        ...
        const estadoLabel=item.estado==='ok'?'<span class="cola-item-estado ok"><i class="fa fa-check"></i> Subido</span>':item.estado==='error'?`<span class="cola-item-estado error"><i class="fa fa-times"></i> ${item.mensaje||'Error'}</span>`:item.estado==='uploading'?'<span class="cola-item-estado uploading"><i class="fa fa-spinner fa-spin"></i> Subiendo...</span>':'<span class="cola-item-estado pending"><i class="fa fa-clock"></i> Pendiente</span>';
        const dis=item.estado==='ok'||item.estado==='uploading'?'disabled':'';
        div.innerHTML+=`<div class="cola-item ${estadoClass}" id="colaItem_${idx}"><div class="cola-item-header"><i class="fa fa-file-circle-check" style="color:#6b7280;font-size:16px;"></i><span class="cola-item-nombre" title="${item.file.name}">${item.file.name}</span>${estadoLabel}${item.estado!=='ok'&&item.estado!=='uploading'?`<button class="cola-item-remove" onclick="quitarDeCola(${idx})"><i class="fa fa-times"></i></button>`:''}</div><div class="cola-item-inputs"><div><input type="text" id="colaCodigo_${idx}" value="${item.codigo}" placeholder="Código (ej: SLC-001_1)" oninput="colaFiles[${idx}].codigo=this.value;validarCodigoCola(${idx})" onblur="validarCodigoCola(${idx})" ${dis} style="font-family:monospace;font-weight:700;text-transform:uppercase;"><div class="cola-hint" id="colaCodigoHint_${idx}">Formato: SLC-NNN_L</div></div><div><input type="text" id="colaDesc_${idx}" value="${item.descripcion}" placeholder="Descripción (opcional)" oninput="colaFiles[${idx}].descripcion=this.value" ${dis}></div></div></div>`;
    });
}
...
function validarCodigoCola(idx){
    const input=document.getElementById(`colaCodigo_${idx}`),hint=document.getElementById(`colaCodigoHint_${idx}`);
    if(!input||!hint)return;const val=input.value.trim().toUpperCase();colaFiles[idx].codigo=val;input.value=val;
    ...
    input.className='';hint.innerHTML='<i class="fa fa-spinner fa-spin" style="color:#9ca3af;font-size:11px;"></i> Verificando...';
    clearTimeout(validarTimeouts[idx]);
    validarTimeouts[idx]=setTimeout(()=>{
        fetch(`ubicaciones_api.php?accion=validar_codigo&codigo=${encodeURIComponent(val)}`).then(r=>r.json()).then(res=>{
...
async function subirTodos(){
    let hayError=false;
    for(let i=0;i<colaFiles.length;i++){const item=colaFiles[i];if(item.estado==='ok')continue;const val=(item.codigo||'').trim().toUpperCase();if(!val||!/^SLC-\d{3}_\d+$/.test(val)){mostrarToastUbic(`❌ Archivo #${i+1} código inválido`,'error');document.getElementById(`colaCodigo_${i}`)?.focus();hayError=true;break;}const dup=colaFiles.some((c,j)=>j!==i&&c.codigo===val);if(dup){mostrarToastUbic(`❌ Código duplicado: ${val}`,'error');hayError=true;break;}}
    if(hayError)return;
    const pendientes=colaFiles.filter(c=>c.estado!=='ok');if(!pendientes.length){mostrarToastUbic('Todos ya subidos','info');return;}
    ...
    wrap.style.display='block';btn.disabled=true;let subidos=0;const total=pendientes.length;
    for(let i=0;i<colaFiles.length;i++){
        const item=colaFiles[i];if(item.estado==='ok')continue;
        item.estado='uploading';renderCola();info.textContent=`Subiendo ${subidos+1}/${total}: ${item.file.name}`;
        const fd=new FormData();fd.append('accion','subir');fd.append('id_socio',document.getElementById('subirIdSocio').value);fd.append('archivo',item.file);fd.append('codigo_archivo',item.codigo.toUpperCase());fd.append('descripcion',item.descripcion);
        try{const res=await fetch('ubicaciones_api.php',{method:'POST',body:fd});const json=await res.json();if(json.success){item.estado='ok';subidos++;}else{item.estado='error';item.mensaje=json.message||'Error';}}catch(e){item.estado='error';item.mensaje='Error de red';}
    ...
    else{mostrarToastUbic(`✅ ${subidos} archivo(s) subidos`,'success');setTimeout(()=>{colaFiles=colaFiles.filter(c=>c.estado!=='ok');if(!colaFiles.length){document.getElementById('colaArchivos').style.display='none';document.getElementById('bulkActions').style.display='none';document.getElementById('bulkProgressWrap').style.display='none';}else{renderCola();}},2000);}
}
...
function cargarKmlEnMapa(idUbicacion,nombreArchivo){
    const div=document.getElementById('mapaContainer');div.innerHTML='<div class="mapa-placeholder"><i class="fa fa-spinner fa-spin"></i><span>Cargando KML...</span></div>';
    fetch(`ubicaciones_api.php?accion=leer_kml&id_ubicacion=${idUbicacion}`).then(r=>r.json()).then(data=>{
        ...
        const kmlContent=atob(data.kml);div.innerHTML='<div id="mapaLeaflet" style="width:100%;height:100%;"></div>';
        cargarLeafletConCallback(kmlContent,idUbicacion,nombreArchivo||data.nombre);
    });
}
function asegurarMapa(){
    if(!mapaLeaflet){const c=document.getElementById('mapaContainer');c.innerHTML='<div id="mapaLeaflet" style="width:100%;height:100%;"></div>';mapaLeaflet=L.map('mapaLeaflet',{zoomControl:true}).setView([-1.5,-79.5],9);tileLayerActual=L.tileLayer(tilesDisponibles[modoMapa].url,{attribution:tilesDisponibles[modoMapa].att}).addTo(mapaLeaflet);setTimeout(()=>{if(mapaLeaflet)mapaLeaflet.invalidateSize();},300);}
}
function cargarLeafletConCallback(kmlContent,idUbicacion,nombreArchivo){
    ...
    if(!window.L){const js=document.createElement('script');js.src='https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.js';js.onload=()=>{const js2=document.createElement('script');js2.src='https://cdnjs.cloudflare.com/ajax/libs/leaflet-omnivore/0.3.4/leaflet-omnivore.min.js';js2.onload=()=>agregarCapaKml(kmlContent,idUbicacion,nombreArchivo);document.head.appendChild(js2);};document.head.appendChild(js);}
...
function agregarCapaKml(kmlContent,idUbicacion,nombreArchivo){
    asegurarMapa();const idx=Object.keys(capasKml).length%COLORES_KML.length;const color=COLORES_KML[idx];
    const layer=omnivore.kml.parse(kmlContent,null,L.geoJson(null,{style:{color,weight:2.5,fillOpacity:0.3,fillColor:color},pointToLayer:(f,latlng)=>L.circleMarker(latlng,{radius:7,fillColor:color,color:'#fff',weight:2,fillOpacity:0.9})}))
    .on('ready',function(){try{const ab=Object.values(capasKml).map(c=>c.layer.getBounds()).filter(b=>b.isValid());if(ab.length>0){let cb=ab[0];ab.forEach(b=>{cb=cb.extend(b);});mapaLeaflet.fitBounds(cb,{padding:[30,30]});}}catch(e){}setTimeout(()=>{if(mapaLeaflet)mapaLeaflet.invalidateSize();},200);})
    .on('error',function(e){console.warn('KML:',e);}).addTo(mapaLeaflet);
    capasKml[idUbicacion]={layer,nombre:nombreArchivo,color,activa:true};actualizarChipsCapas();
}
function actualizarChipsCapas(){
    const div=document.getElementById('listaCapasMapa');if(!div)return;div.innerHTML='';
    Object.entries(capasKml).forEach(([id,c])=>{const chip=document.createElement('div');chip.style.cssText=`display:inline-flex;align-items:center;gap:6px;background:#f9fafb;border:2px solid ${c.color};border-radius:20px;padding:4px 10px;font-size:12px;font-weight:600;cursor:pointer;user-select:none;`;chip.title='Clic para ocultar/mostrar';chip.innerHTML=`<span style="width:10px;height:10px;border-radius:50%;background:${c.color};display:inline-block;flex-shrink:0;"></span>${c.nombre}<i class="fa fa-times" style="color:#9ca3af;margin-left:4px;cursor:pointer;"></i>`;chip.querySelector('.fa-times').addEventListener('click',(e)=>{e.stopPropagation();quitarCapaKml(id);});chip.addEventListener('click',()=>toggleCapaKml(id));div.appendChild(chip);});
}
...
function mostrarTodosKml(){
    ...
    fetch(`ubicaciones_api.php?accion=listar&id_socio=${socioActual.idSocio}`).then(r=>r.json()).then(data=>{
        if(!data.success||!data.datos.length){mostrarToastUbic('No hay archivos KML','error');return;}
        Object.keys(capasKml).forEach(id=>{if(capasKml[id]&&mapaLeaflet)mapaLeaflet.removeLayer(capasKml[id].layer);});capasKml={};actualizarChipsCapas();
        const div=document.getElementById('mapaContainer');if(!mapaLeaflet||!document.getElementById('mapaLeaflet')){div.innerHTML='<div id="mapaLeaflet" style="width:100%;height:100%;"></div>';mapaLeaflet=null;}
        let cargados=0;data.datos.forEach(a=>{fetch(`ubicaciones_api.php?accion=leer_kml&id_ubicacion=${a.id_ubicacion}`).then(r=>r.json()).then(d=>{if(d.success)cargarLeafletConCallback(atob(d.kml),a.id_ubicacion,a.codigo_archivo||a.nombre_archivo);cargados++;if(cargados===data.datos.length)mostrarToastUbic(`✅ ${data.datos.length} cargados`,'success');});});
    });
}
...
function eliminarArchivo(idUbicacion){
    mostrarConfirmacion('Eliminar archivo','¿Eliminar este archivo? Esta acción no se puede deshacer.',function(){
        fetch('ubicaciones_api.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`accion=eliminar&id_ubicacion=${idUbicacion}`}).then(r=>r.json()).then(j=>{
...
function renderPaginacion(pagina,totalPaginas,total,porPagina){
    ...
    for(let p=Math.max(1,pagina-2);p<=Math.min(totalPaginas,pagina+2);p++)html+=`<button onclick="cargarSocios(${p})" class="${p===pagina?'active':''}">${p}</button>`;
...
function mostrarToastUbic(msg,tipo){
    ...
    t.textContent=msg;t.style.background=tipo==='success'?'#10b981':tipo==='error'?'#ef4444':'#1f3a5f';
    t.style.transform='translateY(0)';t.style.opacity='1';setTimeout(()=>{t.style.transform='translateY(80px)';t.style.opacity='0';},3500);
}
...
function cerrarResumenGlobal(){document.getElementById('modalResumenGlobal').classList.remove('active');}

async function cargarResumenGlobal(){
    document.getElementById('rgContenidoSocios').innerHTML='<div class="rg-loading"><i class="fa fa-spinner fa-spin" style="font-size:24px;color:#4f46e5;display:block;margin-bottom:8px;"></i>Cargando datos...</div>';
    document.getElementById('rgContenidoLibres').innerHTML='<div class="rg-loading"><i class="fa fa-spinner fa-spin" style="font-size:24px;color:#10b981;display:block;margin-bottom:8px;"></i>Calculando códigos...</div>';
    try{
        const r=await fetch('ubicaciones_resumen_global.php');const txt=await r.text();let data;
        try{data=JSON.parse(txt);}catch(e){document.getElementById('rgContenidoSocios').innerHTML=`<div class="rg-loading" style="color:#ef4444;">❌ Error del servidor:<br><small style="font-family:monospace;">${txt.substring(0,300)}</small></div>`;return;}
        if(!data.success){document.getElementById('rgContenidoSocios').innerHTML=`<div class="rg-loading" style="color:#ef4444;">❌ ${data.message}</div>`;return;}
        rgDatos=data.socios||[];
        document.getElementById('rgStTotal').textContent=data.stats.total_socios;document.getElementById('rgStConKml').textContent=data.stats.socios_con_kml;
        document.getElementById('rgStSinKml').textContent=data.stats.socios_sin_kml;document.getElementById('rgStHa').textContent=data.stats.total_ha>0?data.stats.total_ha.toFixed(2)+' ha':'—';
        document.getElementById('rgStLibres').textContent=data.stats.codigos_libres;
        rgCargado=true;renderResumenGlobalSocios();renderResumenGlobalLibresDesdeData(data.codigos_libres,data.socios);
    }catch(e){document.getElementById('rgContenidoSocios').innerHTML=`<div class="rg-loading" style="color:#ef4444;">❌ Error de red: ${e.message}</div>`;}
}

function renderResumenGlobalSocios(){
    const q=rgFiltro.toLowerCase();let datos=rgDatos;
    if(q)datos=rgDatos.filter(s=>s.nombre.toLowerCase().includes(q)||s.identificacion.toLowerCase().includes(q)||s.lotes.some(l=>(l.codigo||'').toLowerCase().includes(q)));
    if(!datos.length){document.getElementById('rgContenidoSocios').innerHTML=`<div class="rg-loading" style="color:#9ca3af;">Sin resultados para "${escHtmlRg(rgFiltro)}"</div>`;return;}
    let filas='';
    datos.forEach((s,idx)=>{
        const haStr=s.totalHa>0?`<span class="rg-ha">${parseFloat(s.totalHa).toFixed(3)} ha</span>`:`<span class="rg-ha-nd">Sin datos</span>`;
        const codigosHtml=s.lotes.length?s.lotes.map(l=>`<span class="rg-cod">${escHtmlRg(l.codigo)}</span>`).join(''):`<span class="rg-no-arch">Sin archivos</span>`;
        const adBadge=s.adendum===2?'<span style="background:#fef9c3;color:#854d0e;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;">Ad.2</span>':s.adendum===1?'<span style="background:#dbeafe;color:#1d4ed8;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;">Ad.1</span>':'<span style="background:#f3f4f6;color:#9ca3af;padding:2px 8px;border-radius:999px;font-size:10px;">—</span>';
        filas+=`<tr><td style="font-weight:600;color:#6b7280;text-align:center;">${idx+1}</td><td style="font-family:monospace;font-size:12px;font-weight:700;">${escHtmlRg(s.identificacion)}</td><td><strong>${escHtmlRg(s.nombre)}</strong><br><small style="color:#9ca3af;">${escHtmlRg(s.zona)}</small></td><td style="text-align:center;">${adBadge}</td><td style="text-align:center;">${haStr}</td><td style="text-align:center;font-weight:700;color:#4f46e5;">${s.lotes.length}</td><td><div class="rg-cod-list">${codigosHtml}</div></td></tr>`;
    });
    const totalHaFiltrado=datos.reduce((s,x)=>s+(x.totalHa||0),0),totalLotes=datos.reduce((s,x)=>s+x.lotes.length,0);
    document.getElementById('rgContenidoSocios').innerHTML=`<table class="rg-table"><thead><tr><th style="width:36px;">#</th><th>Cédula</th><th>Nombre</th><th style="text-align:center;">Adendum</th><th style="text-align:center;">Hectáreas</th><th style="text-align:center;">Lotes</th><th>Códigos</th></tr></thead><tbody>${filas}</tbody><tfoot><tr style="background:#f1f5f9;font-weight:700;"><td colspan="4" style="padding:10px;text-align:right;font-size:12px;">TOTAL ${datos.length} socios:</td><td style="text-align:center;font-family:monospace;color:#1f3a5f;padding:10px;">${totalHaFiltrado>0?totalHaFiltrado.toFixed(3)+' ha':'—'}</td><td style="text-align:center;padding:10px;">${totalLotes}</td><td></td></tr></tfoot></table>`;
}
...
function filtrarResumenGlobal(){
    rgFiltro=document.getElementById('rgBuscar').value||'';renderResumenGlobalSocios();
    const usados=new Set();let maxNum=0;
    rgDatos.forEach(s=>s.lotes.forEach(l=>{const m=(l.codigo||'').match(/^SLC-(\d+)/i);if(m){const n=parseInt(m[1]);usados.add(n);if(n>maxNum)maxNum=n;}}));
    const libresActuales=[];for(let i=1;i<=maxNum;i++){if(!usados.has(i))libresActuales.push('SLC-'+String(i).padStart(3,'0'));}
    renderResumenGlobalLibresDesdeData(libresActuales,rgDatos);
}

function cambiarRgTab(tabId,btn){document.querySelectorAll('.rg-tc').forEach(t=>t.classList.remove('active'));document.querySelectorAll('.rg-tab').forEach(b=>b.classList.remove('active'));document.getElementById(tabId).classList.add('active');if(btn)btn.classList.add('active');}
function escHtmlRg(s){return(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}

function exportarResumenGlobalExcel(){
    const q=rgFiltro.toLowerCase();const datos=q?rgDatos.filter(s=>s.nombre.toLowerCase().includes(q)||s.identificacion.toLowerCase().includes(q)):rgDatos;
    if(!datos.length){mostrarToastUbic('No hay datos para exportar','error');return;}
    const lotes=[];datos.forEach(s=>{if(s.lotes.length){s.lotes.forEach(l=>{lotes.push({cedula:s.identificacion,nombre:s.nombre,zona:s.zona,comunidad:s.comunidad,adendum:s.adendum===2?'Adendum 2':s.adendum===1?'Adendum 1':'—',codigo:l.codigo,hectareas:l.hectareas});});}else{lotes.push({cedula:s.identificacion,nombre:s.nombre,zona:s.zona,comunidad:s.comunidad,adendum:s.adendum===2?'Adendum 2':s.adendum===1?'Adendum 1':'—',codigo:'',hectareas:null});}});
...
function cerrarComparacion(){document.getElementById('modalComparacion').classList.remove('active');}
...
async function cargarComparacion(){
    ...
    try{
        ...
        icDatos=(dataGlobal.socios||[]).map(s=>{
            ...
            else if(areaCacao===null&&haKml!==null)estado='sin-lpa';
            return{id_socio:s.id_socio,identificacion:s.identificacion,nombre:s.nombre,zona:s.zona||'',adendum:s.adendum,area_cacao_ha:areaCacao,ha_kml:haKml,total_archivos:s.lotes?s.lotes.length:0,diferencia:diff,estado};
        });
        icCargado=true;
...
function setFiltroIc(btn,filtro){document.querySelectorAll('.ic-filtro-btn').forEach(b=>b.classList.remove('activo'));btn.classList.add('activo');icFiltroEstado=filtro;filtrarComparacion();}

function filtrarComparacion(){
    const q=(document.getElementById('icBuscar').value||'').toLowerCase().trim();
    icDatosFiltro=icDatos.filter(d=>{if(icFiltroEstado!=='todos'&&d.estado!==icFiltroEstado)return false;if(q&&!d.identificacion.toLowerCase().includes(q)&&!d.nombre.toLowerCase().includes(q))return false;return true;});
...
function renderTablaComparacion(){
    ...
    icDatosFiltro.forEach((d,idx)=>{
        ...
        const haKmlStr=d.ha_kml!==null?`<span class="ic-ha">${d.ha_kml.toFixed(3)}</span>`:`<span class="ic-ha-nd">Sin KML</span>`;
        let diffHtml=`<span class="ic-ha-nd">—</span>`;
    ...
    const totalLpa=icDatosFiltro.reduce((s,d)=>s+(d.area_cacao_ha||0),0),totalKml=icDatosFiltro.reduce((s,d)=>s+(d.ha_kml||0),0);
    cont.innerHTML=`<table class="ic-table"><thead><tr><th style="width:36px;">#</th><th>Cédula</th><th>Productor/a</th><th style="text-align:center;">Adendum</th><th style="text-align:center;">Área Cacao LPA (ha)</th><th style="text-align:center;">Total Ha KML</th><th style="text-align:center;">Archivos</th><th>Diferencia</th><th style="text-align:center;">Estado</th><th style="text-align:center;">Igualar</th></tr></thead><tbody>${filas}</tbody><tfoot><tr style="background:#f1f5f9;font-weight:700;"><td colspan="4" style="padding:10px 12px;text-align:right;font-size:12px;color:#374151;">TOTALES (${icDatosFiltro.length} socios):</td><td style="text-align:center;padding:10px 12px;font-family:monospace;color:#0f766e;">${totalLpa>0?totalLpa.toFixed(3)+' ha':'—'}</td><td style="text-align:center;padding:10px 12px;font-family:monospace;color:#0f766e;">${totalKml>0?totalKml.toFixed(3)+' ha':'—'}</td><td></td><td style="text-align:center;padding:10px 12px;font-family:monospace;font-weight:700;color:${(totalKml-totalLpa)>=0?'#f59e0b':'#ef4444'};">${totalLpa>0||totalKml>0?(totalKml-totalLpa>0?'+':'')+(totalKml-totalLpa).toFixed(3)+' ha':'—'}</td><td colspan="2"></td></tr></tfoot></table>`;
}

async function igualerHa(idSocio,haKml){
    mostrarConfirmacion('Igualar Área Cacao',`¿Actualizar el Área Cacao (Ha) en LPA a ${parseFloat(haKml).toFixed(3)} ha?`,async function(){
        try{
            const res=await fetch('lpa_igualar.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`id_socio=${encodeURIComponent(idSocio)}&area_cacao_ha=${encodeURIComponent(haKml)}`});
            const j=await res.json();
            if(j.success){mostrarToastUbic(`✅ Área actualizada a ${parseFloat(haKml).toFixed(3)} ha`,'success');icCargado=false;await cargarComparacion();}
            else{mostrarToastUbic('❌ '+(j.message||'Error al actualizar'),'error');}
        }catch(e){mostrarToastUbic('❌ Error de red','error');}
    });
}

async function igualerTodos(){
    const pendientes=icDatosFiltro.filter(d=>d.ha_kml!==null&&d.estado!=='igual');
    if(!pendientes.length){mostrarToastUbic('No hay socios para igualar','info');return;}
    mostrarConfirmacion('Igualar todos',`¿Actualizar el Área Cacao en LPA para ${pendientes.length} productores con diferencia?`,async function(){
        let ok=0,err=0;
        for(const d of pendientes){
            try{const res=await fetch('lpa_igualar.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`id_socio=${encodeURIComponent(d.id_socio)}&area_cacao_ha=${encodeURIComponent(d.ha_kml)}`});const j=await res.json();j.success?ok++:err++;}
            catch(e){err++;}
        }
        mostrarToastUbic(`✅ ${ok} igualados${err?', ❌ '+err+' errores':''}`,'success');
        icCargado=false;await cargarComparacion();
    });
}
</script>
</body>
